...
export default function VerNotas() {
    ...
    const [notePost, setNotePost] = useState<Array<{
        nota_coeficiente_rendimento: string;
        nota_entrevista: string;
        situacao: 'Habilitado' | 'Inabilitado' | 'Desclassificado';
        motivo_situacao: string;
    ...
    useEffect(() => {
        const fetchVacancy = async () => {
            try {
                ...
                const notesData = Array.isArray(responseNotes.data) ? responseNotes.data : [responseNotes.data];
        ...
        fetchVacancy();
    }, [candidacyId, candidatoId, vacancyId]);

    const handleCreateNote = async (e: React.FormEvent) => {
        e.preventDefault();

        try {
            const coeficienteRendimento = parseFloat(notePost[0]?.nota_coeficiente_rendimento || '0');
            if (coeficienteRendimento > 10.0 || coeficienteRendimento < 0.0) {
                alert('Nota Coeficiente Rendimento deve ser entre 0.0 e 10.0');
                return;
            }

            const entrevista = parseFloat(notePost[0]?.nota_entrevista || '0');
            if (entrevista > 50.0 || entrevista < 0.0) {
                alert('Nota Entrevista deve ser entre 0.0 e 50.0')
                return;
            }

            await axios.post(`http://localhost:8000/api/admin/${adminId}/candidacy/${candidacyId}/classification`, notePost[0]);
            ...
            {note[0]?.id ? (
                    ...
                    <div>
                        ...
                        {note[0]?.situacao === 'Desclassificado' && (
                            ...
                            <Input
                                ...
                                const updatedNote = [...note];
                                if (updatedNote[0]) {
                                    updatedNote[0].motivo_situacao = value;
                                }
                                setNote(updatedNote);
                                }}
                ...
                <div>
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input
                        id="coeficiente_rendimento"
                        value={notePost[0]?.nota_coeficiente_rendimento || ''}
                        onChange={(e) => {
                            const value = e.target.value;
                            if (/^\d{0,2}(\.\d{0,2})?$/.test(value)) {
                            const updatedNotePost = [...notePost];
                            if (updatedNotePost[0]) {
                                updatedNotePost[0].nota_coeficiente_rendimento = value;
                            }
                            setNotePost(updatedNotePost);
                            }
                        }}
                        placeholder="Nota Coeficiente Rendimento: 0.0 - 10.0"
                        />
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input
                        id="entrevista"
                        value={notePost[0]?.nota_entrevista || ''}
                        onChange={(e) => {
                            const value = e.target.value;
                            if (/^\d{0,2}(\.\d{0,2})?$/.test(value)) {
                            const updatedNotePost = [...notePost];
                            if (updatedNotePost[0]) {
                                updatedNotePost[0].nota_entrevista = value;
                            }
                            setNotePost(updatedNotePost);
                            }
                        }}
                    ...
                    <div className="grid gap-2">
                        <Label htmlFor="situacao">Situação</Label>
                        <Select
                        value={notePost[0]?.situacao || ''}
                        onValueChange={(value) => {
                            const updatedNotePost = [...notePost];
                            if (updatedNotePost[0]) {
                            updatedNotePost[0].situacao = value as 'Habilitado' | 'Inabilitado' | 'Desclassificado';
                            }
                            setNotePost(updatedNotePost);
                        }}
                        required
                    ...
                    {notePost[0]?.situacao === 'Desclassificado' && (
                        <div className="grid gap-2">
                        <Label htmlFor="motivo_situacao">Motivo da Situação</Label>
                        <Input
                            id="motivo_situacao"
                            value={notePost[0]?.motivo_situacao || ''}
                            onChange={(e) => {
                            const value = e.target.value;
                            const updatedNotePost = [...notePost];
                            if (updatedNotePost[0]) {
                                updatedNotePost[0].motivo_situacao = value;
                            }
                            setNotePost(updatedNotePost);
                            }}
                            placeholder="Informe o motivo da situação"
                    ...
                    <Button className='hover:cursor-pointer bg-green-600 hover:bg-green-700'>Cadastrar Notas <Plus /></Button>
                    </form>
                </div>
            )}
        </AppLayout>
