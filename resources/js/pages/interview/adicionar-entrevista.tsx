...
export default function AdicionarEntrevista() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/admin/${adminId}/candidacy/${candidacyId}/interview`, {
                method: 'POST',
                headers: {
            ...
            if (res.ok) {
                setMensagem('Entrevista cadastrada com sucesso!');
                setData('');
                setHora('');
                setLocal('');
                setStatus('Agendada');
                ...
                setMensagem(err.message || 'Erro ao cadastrar entrevista');
            }
        } catch {
            ...
            <div className="flex h-full max-h-full flex-1 flex-col  rounded-xl p-4">
                <nav className="text-sm text-muted-foreground ">
                    <ol className="flex items-center space-x-2">
                        ...
                        <li>
                            <span className=" text-[#008DD0] mx-1 ">/</span>
                            <span className=" text-[#008DD0]">Entrevista Candidato</span>
                        </li>
                        <li>
                            <span className=" text-[#008DD0] mx-1 ">/</span>
                            <span className="font-medium text-[#008DD0]">Adiconar Entrevista</span>
                        </li>
                    </ol>
                </nav>
                 <div className="mt-2 mb-1 w-fit">
                    <h1 className="text-2xl text-black">Adicionar Entrevista</h1>
